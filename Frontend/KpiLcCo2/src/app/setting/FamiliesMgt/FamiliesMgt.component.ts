...
import { FamiliesMgtModalComponent } from '../families-mgt-modal/families-mgt-modal.component';
...
@Component({
  selector: 'app-users',
  templateUrl: './FamiliesMgt.component.html',
  styleUrls: ['./FamiliesMgt.component.scss'],  
})

export class FamiliesMgtComponent implements OnInit {
  ...
  tableRows: RefTranscoFpWex[];
  selectedSession: RefTranscoFpWex[] = [];
  ...
  breadcrumbs: TranslateText[] = [
    ...
    { key: 'nav.FamiliesMgt', defaultText: 'Families management' },
  ];
  ...
  ngOnInit() {
    this.tableColumns = [
      { name: 'Family', prop: 'family', sortable: false },
      { name: 'Label', prop: 'label12', sortable: false },
      { name: 'Owner Country', prop: 'country', sortable: false },
      { name: 'ARR. Country', prop: 'arrCountry', sortable: false },
      { name: 'Xaccount Id', prop: 'xaccountId', sortable: false },
      { name: 'Spr Id', prop: 'sprId', sortable: false },
    ];
  ...
  getUsers(resetParams = false) {
    ...
    this.loaderService.addRequest(this.usersService.getAllUplet())
      .subscribe(
        (data: any) => {
          this.tableRows = data;
  ...
  showAddEditModal(editRow?: any) {
    ...
    const modalRef = this.modalsService.addEditEntity(FamiliesMgtModalComponent, { windowClass: 'save-user-modal' });

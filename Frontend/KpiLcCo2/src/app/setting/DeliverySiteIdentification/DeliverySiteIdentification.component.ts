...
import { Platformdata, ReportSetting, User,countrydata } from 'src/app/shared/models/auth.model';
...
@Component({
  selector: 'app-users',
  templateUrl: './DeliverySiteIdentification.component.html',
  styleUrls: ['./DeliverySiteIdentification.component.scss'],
  //styles: ['.form-group { margin-bottom: 16px; }', '.checkbox { margin-top: inherit }'],
  //encapsulation: ViewEncapsulation.Emulated
})
export class DeliverySiteIdentificationComponent implements OnInit {
  ...
  breadcrumbs: TranslateText[]  = [
    ...
    { key: 'nav.DeliverySiteIdentification', defaultText: 'To set-up for delivery site identification' },
  ];
...
pageSize = 10;
  ...
  getUsers(resetParams = false) {
    ...
    this.loaderService.addRequest(this.usersService.getDelayReport(ReportSetting.ToSetUpForDelivery))
      .subscribe(
        (data:any) => {
  ...
  showAddEditModal(editRow?: any) {
    ...
    const modalRef = this.modalsService.addEditEntity(DeliverySiteIdentificationModalComponent, { windowClass: 'save-user-modal' });
    ...
    modalRef.result.then(response => {
      if (typeof response !== 'string') {
        ...
        if (editMode) {
          ...
          location.reload();
          this.getUsers();
          // this.tableRows = updateArrAtIndex(this.tableRows, rowIndex, newRowObj);
        } else {
         // this.getUsers();
        }
      }
  ...
  //  const {rowinfo}=
    ...
    modalRef.componentInstance.deleteConfirmed.subscribe(() => {
      ...
      this.loaderService.addRequest(this.usersService.deleteReportParams(userId))
      .subscribe(response => {
        console.log(response);
        modalRef.close();
        this.getUsers();

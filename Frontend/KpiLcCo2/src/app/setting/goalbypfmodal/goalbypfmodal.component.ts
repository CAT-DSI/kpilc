...
import { PlateformesModel, ReportOpenDaysModel, UserForm, UserRoles } from 'src/app/shared/models/auth.model';
...
@Component({
  selector: 'app-goalbypfmodal',
  templateUrl: './goalbypfmodal.component.html',
  styleUrls: ['./goalbypfmodal.component.scss']
})
export class GoalbypfmodalComponent implements OnInit, OnDestroy {
  @Input() headerTitle: TranslateText;
  @Input() formData = {} as PlateformesModel;
  countryList: Country[];
  ...
  ngOnInit() {
    this.callapi = false;
    const { id, country, codePlf,plateform,euro, nbr,isCat, display } = this.formData;
    ...
    this.isCat=this.userForm.value.isCat;
    this.display=this.userForm.value.display;
    ...
    const dateValue = this.userForm.value.month;    
    const formatedDate = this.formatDate(dateValue);
    this.userForm.get('month')?.setValue(formatedDate);
  ...
  onSubmit() {
    if (true) {      
      ...
      data.codePlf = data.codePlf;
      data.plateform = data.plateform;
      data.euro = data.euro;
      data.nbr = data.nbr;
      ...
      this.loaderService.addRequest(this.usersService.updatePlateForme(data))
        .subscribe(

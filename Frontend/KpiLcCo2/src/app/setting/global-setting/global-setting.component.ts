...
import { countrydata, PlateformesModel, Platformdata, User } from 'src/app/shared/models/auth.model';
...
import { GoalbypfmodalComponent } from '../goalbypfmodal/goalbypfmodal.component';
...
export class GlobalSettingComponent implements OnInit {
  ...
  @ViewChild('chkboxColCellDisplay', { static: true }) chkboxColCellDisplay: TemplateRef<any>;
  ...
  breadcrumbs: TranslateText[] = [
    ...
    { key: 'nav.GoalbyPF', defaultText: 'Goal by PF' },
  ];
  ...
  ngOnInit() {
    this.tableColumns = [
      ...
      { name: 'CodePLF', prop: 'codePlf', sortable: false },
      { name: 'Plateform', prop: 'plateform', sortable: false },
      { name: 'IsCAT', prop: 'isCat', sortable: false , cellTemplate: this.chkboxColCellTmpl},
      { name: 'NBR', prop: 'nbr', sortable: false },
      { name: 'Euro', prop: 'euro', sortable: false },     
      { name: 'Display', prop: 'display', sortable: false, cellTemplate: this.chkboxColCellDisplay },
    ];
...
this.getUsers();
  ...
  showAddEditModal(editRow?: any) {
    ...
    const modalRef = this.modalsService.addEditEntity(GoalbypfmodalComponent, { windowClass: 'save-user-modal' });
  ...
  getOpenDays(e) {
    ...
    debugger
    this.loaderService.addRequest(this.usersService.getPlateformesUCountry(this.profiledata))
      .subscribe(
        (data: any) => {
          debugger
          ...
          this.tableRows.filter(item => item.isCat);

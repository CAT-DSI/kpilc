...
import { Platformdata, ReportCategoriesPfsModel, User,countrydata } from 'src/app/shared/models/auth.model';
...
@Component({
  selector: 'app-users',
  templateUrl: './AttributionDetails.component.html',
  styleUrls: ['./AttributionDetails.component.scss'],
  //styles: ['.form-group { margin-bottom: 16px; }', '.checkbox { margin-top: inherit }'],
  //encapsulation: ViewEncapsulation.Emulated
})
export class AttributionDetailsComponent implements OnInit {
  ...
  tableRows: ReportCategoriesPfsModel[];
  selectedSession: ReportCategoriesPfsModel[]=[];  
  ...
  breadcrumbs: TranslateText[]  = [
    ...
    { key: 'nav.AttributionDetails', defaultText: 'Attribution PF aux rapports délais' },
  ];
...
pageSize = 10;
  ...
  ngOnInit() {
    ...
    this.tableColumns =[ 
      ...
      { name: 'PF', prop: 'pf',sortable: false  },
  ...
  getUsers(resetParams = false) {
    ...
    this.loaderService.addRequest(this.usersService.getAllCategoriesPFByCategory(1))
      .subscribe(
        (data:any) => {        
          this.tableRows = data;
...
debugger
    ...
    const modalRef = this.modalsService.addEditEntity(AttributionDetailsModalComponent, { windowClass: 'save-user-modal' });
  ...
  deleteUser(editRow: any) {    
    ...
    let use = this.tableRows.filter(x=>x.catagoryId==editRow.catagoryId && x.country == editRow.country && x.pf == editRow.pf);
     const entityValue = use[0].pf ;    
    ...
    modalRef.componentInstance.deleteConfirmed.subscribe(() => {
      const userId = use[0].catagoryId;
      this.loaderService.addRequest(this.usersService.deleteReportWeb_CategoryPF(editRow))
      .subscribe(response => {       
        this.getUsers();
        modalRef.close();
    },

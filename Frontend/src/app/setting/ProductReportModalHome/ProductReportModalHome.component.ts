 ...
 */
  @Component({
    ...
    templateUrl: './ProductReportModalHome.component.html',
    styleUrls: ['./ProductReportModalHome.component.scss']
  
  })
  
  export class ProductReportModalHome  implements OnInit, OnDestroy {
...
changeCountry(e) {
  ...
  onSubmit() {
    if (true) {
     ...
     data.regimeId=data.regime.id;
     data.product=data.product.value;
     data.regime=data.regime.regime;
    
     // if userId is defined then it will update else it will create new
     this.loaderService.addRequest(this.usersService.AddUpdateProductWithKpi(data))

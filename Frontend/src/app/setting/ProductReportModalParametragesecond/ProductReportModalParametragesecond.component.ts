...
import { KpiSettingModel, Productreport, ReportSetting,User, UserForm, UserRoles, delayreport } from 'src/app/shared/models/auth.model';
 ...
 */
  @Component({
    ...
    templateUrl: './ProductReportModalParametragesecond.component.html',
    styleUrls: ['./ProductReportModalParametragesecond.component.scss']
  
  })
  
  export class ProductReportModalParametragesecond implements OnInit, OnDestroy {

  @Input() headerTitle: TranslateText;
  @Input() formData = {} as KpiSettingModel;
  countryList: Country[];
  profilelist:profile[];
 ...
 // this.userForm.controls['profileName'].setValue(this.profiledata, {onlySelf: true});
  ...
  ngOnInit() {
    ...
    this.callapi=false;
    const {kpi,name,id} = this.formData;

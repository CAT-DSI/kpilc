...
import { BasicParams, PagingConfig, RecordStatus, SortTypes } from 'src/app/shared/models/app';
import { SelectionType, id } from '@swimlane/ngx-datatable';
import { Platformdata, ReportsParamsModel, User, countrydata } from 'src/app/shared/models/auth.model';
...
import { getFromStorage, saveToStorage } from 'src/app/utils/storage';
...
@Component({
  selector: 'app-users',
  templateUrl: './OpendaysMgt.component.html',
  styleUrls: ['./OpendaysMgt.component.scss'],
})
export class OpendaysMgtComponent implements OnInit {
  ...
  breadcrumbs: TranslateText[] = [
    ...
    { key: 'nav.OpendaysMgt', defaultText: 'Open days management' },
  ];
  ...
  ngOnInit() {
    this.tableColumns = [
      ...
      { name: 'Day', prop: 'day', sortable: false },
    ];
  ...
  showAddEditModal(editRow?: any) {
    ...
    const modalRef = this.modalsService.addEditEntity(OpendaysMgtModalComponent, { windowClass: 'save-user-modal' });
  ...
  getOpenDays(e) {
    ...
    debugger
    this.loaderService.addRequest(this.usersService.getOpenDaysDetByCountryAndYear(this.profiledata, this.yearData))
      .subscribe(
        (data: any) => {

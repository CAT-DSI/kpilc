...
import { WayBillModalComponent } from '../way-bill-modal/way-bill-modal.component';
...
@Component({
  ...
  templateUrl: './WayBillReport.component.html',
  styleUrls: ['./WayBillReport.component.scss'],
  //styles: ['.form-group { margin-bottom: 16px; }', '.checkbox { margin-top: inherit }'],
  //encapsulation: ViewEncapsulation.Emulated
})
export class WayBillReportComponent implements OnInit {
  ...
  breadcrumbs: TranslateText[]  = [
    ...
    { key: 'nav.ToExcludeCustomersForWayBillReport', defaultText: 'To Exclude Customers For WayBill Report' },
  ];
...
pageSize = 10;
  ...
  getUsers(resetParams = false) {
    ...
    this.loaderService.addRequest(this.usersService.getDelayReport(ReportSetting.ToExcludeCustomers))
      .subscribe(
        (data:any) => {
  ...
  showAddEditModal(editRow?: any) {
    ...
    const modalRef = this.modalsService.addEditEntity(WayBillModalComponent, { windowClass: 'save-user-modal' });

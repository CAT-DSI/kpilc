...
import { OpenDaysDetModel, ReportOpenDaysModel, UserForm, UserRoles } from 'src/app/shared/models/auth.model';
...
@Component({
  selector: 'app-opendaysmodal',
  templateUrl: './opendaysmodal.component.html',
  styleUrls: ['./opendaysmodal.component.scss']
})
export class OpendaysmodalComponent implements OnInit, OnDestroy {
  @Input() headerTitle: TranslateText;
  @Input() formData = {} as ReportOpenDaysModel;
  countryList: Country[];
  ...
  ngOnInit() {
    this.callapi = false;
    const { id, country, month, nbr, openDays } = this.formData;
  ...
  onSubmit() {
    if (true) {
      ...
      const data: ReportOpenDaysModel = {
      ...
      data.nbr = data.nbr;
      data.openDays = data.openDays;

      // if userId is defined then it will update else it will create new
      this.loaderService.addRequest(this.usersService.updateOpenDays(data))
        .subscribe(
